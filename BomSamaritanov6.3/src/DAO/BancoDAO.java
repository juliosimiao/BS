...
public class BancoDAO implements InterfaceBancoDAO {
    ...
    public void salvar() throws BancoDAOException {
        ...
        try {
            ...
            SQL = "INSERT INTO Processo (EntrevistadoraUm, EntrevistadoraDois,EntrevistadoraInternamento,EntrevistadoraSaida, DataEntrevistaUm, DataEntrevistaDois,DataEntrevistaInternamento,DataEntrevistaSaida, key_int) "
                    + "values (?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
//javax.swing.JOptionPane.showMessageDialog(null, "Dados salvos com sucesso!");
    ...
    public void atualizar(int codigoInterno) throws BancoDAOException {
        ...
        try {
            ...
            String SQL = "UPDATE Interno SET Nome=?, Idade=?, DataNascimento=?, RG=?, CPF=?,"
                    ...
                    + " Naturalidade = ?, ImagemAntes=?, ImagemDepois=?, ComQuemMoraAtual=?, Sexo=? where key_int = ?";
            ...
            ps_query.setInt(1, modelo.getCodigo());
            rs = ps_query.executeQuery();
            if (!rs.next()) {
                throw new BancoDAOException("Não foi encontrado nenhum "
            ...
            for (Iterator it = modelo.getFamiliaSocial().iterator(); it.hasNext();) {
                ...
                SQL = "UPDATE Familiaesocial SET Parentesco=?, Nome=?, EstadoCivil=?,CasoDeDrogas=?,Idade=?,Profissao=?,Relacionamento=?,TempoCasados=? where key_famsoc = ?";
                //Maioria dos erros está na sql de update, não no sistema
            ...
            /*
             ...
             ps.clearParameters();               
             */
            SQL = "UPDATE processo SET EntrevistadoraUm=?, EntrevistadoraDois=?, EntrevistadoraInternamento=?, EntrevistadoraSaida=?, DataEntrevistaUm=?, DataEntrevistaDois=?,DataEntrevistaInternamento=?,DataEntrevistaSaida=? where key_int = ?";
            //Maioria dos erros está na sql de update, não no sistema
    ...
    public ArrayList<Interno> pesquisar(String nomeCliente) throws BancoDAOException {
        ...
        if (nomeCliente.equals("")) {
            ...
            try {
                conn = this.getConn();
                SQL = "SELECT * FROM Interno WHERE Nome LIKE ?";
                ps = conn.prepareStatement(SQL);
                ...
                do {
                    ...
                    interno = new Interno();
                    interno.setCodigo(Integer.parseInt(rsInterno.getString("Numero")));
